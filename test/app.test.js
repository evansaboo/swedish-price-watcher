...
test('cron endpoint respects scheduler state and interval', async () => {
  const state = createDefaultState();
  let triggerCalls = 0;
  ...
  state.stats.lastRunStartedAt = new Date().toISOString();
  ...
  assert.equal(notDueResponse.json().ran, false);
  assert.equal(notDueResponse.json().reason, 'not-due');
  assert.equal(triggerCalls, 0);

  state.stats.lastRunStartedAt = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString();
  const dueResponse = await app.inject({
    method: 'GET',
    url: '/api/cron'
  });

  assert.equal(dueResponse.statusCode, 200);
  assert.equal(dueResponse.json().ran, true);
